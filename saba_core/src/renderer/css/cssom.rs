...
impl CssParser {
    ...
    pub fn parse_stylesheet(&mut self) -> StyleSheet {
        ...
        sheet.set_rules(self.consume_list_of_values());
        sheet
    }

    fn consume_list_of_values(&mut self) -> Vec<QualifiedRule> {
    ...
    fn consume_selector(&mut self) -> Selector {
        let token = match self.t.next() {
            Some(t) => t,
            None => panic!("should have a token but get None"),
        };
        ...
        match token {
            ...
            CssToken::Delim(delim) => {
                ...
                panic!("Parse error: {:?} is an unsxpected token.", token);
            }
