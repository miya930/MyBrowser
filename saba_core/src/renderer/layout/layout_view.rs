...
impl LayoutView {
    ...
    fn calculate_node_position(
        ...
        if let Some(n) = node {
            ...
            let next_sibiling = n.borrow().next_sibling();
            Self::calculate_node_position(
                &next_sibiling,
                parent_point,
                n.borrow().kind(),
...
mod tests {
    ...
    fn test_text() {
        ...
        let text = root.expect("root should text").borrow().first_child();
        assert!(text.is_some());
        assert_eq!(
    ...
    fn test_display_none() {
        let html = "<html><head><style>body{display:none;}</style></head></html>".to_string();
        let layout_view = create_layout_view(html);
